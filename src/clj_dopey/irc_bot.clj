...
(defn on-incoming-message
  ...
  (let [{text :text, target :target, nick :nick, host :host, command :command}
          incoming-message]
    (log/info (str "Received message from"
                   nick
                   "to"
                   target
                   ":"
    ...
    (if (message-for-me? @dyncfg/bot-nick
                         (-> @dyncfg/configuration
                             :bot
                             :prefix)
                         incoming-message)
      (reply-to-incoming-message connection
                                 text
                                 target
                                 nick
                                 host
