  ...
  (jdbc/query
    ...
    [(str
           "select term, description,
                 ...
                 from dictionary "
           (add-where-clause word use-like)) (update-word word use-like)]))
...
(defn select-word-count
  ...
  (->
    (jdbc/query
      db-spec/dopey-db
      [(str "select count(*) as cnt from dictionary "
                  (add-where-clause word use-like)) (update-word word use-like)])
    first
    :cnt))
...
(defn all-words-count
  []
  (-> (jdbc/query db-spec/dopey-db ["select count(*) as cnt from dictionary"])
      first
      :cnt))
