...
(deftest test-is-word-from-dictionary?-existence
  ...
  (testing
    "if the clj-dopey.irc-bot/is-word-from-dictionary? definition exists."
...
(deftest test-return-words-from-dictionary-existence
  ...
  (testing
    "if the clj-dopey.irc-bot/return-words-from-dictionary definition exists."
...
(deftest test-message-for-me?-1
  ...
  (testing "the function message-for-me?"
           (are [name special-prefix message]
                (message-for-me? name special-prefix {:target message})
                "bot" "?"
                "bot:" "bot"
                "?" "bot: message")))
...
(def test-word
  {:description "For details, see the universal Glossary.",
   :use 1,
   :copyrighted 0,
   :source "The Glossary",
   :internal 0,
...
(deftest test-return-words-from-dictionary
  ...
  (testing "the function return-words-from-dictionary"
           (with-redefs [dictionary/find-word
                           (fn [input] (dictionary/correct-forms test-word))]
             (is (= (return-words-from-dictionary "test")
                    (dictionary/correct-forms test-word))))))
...
(deftest test-return-more-words-like-this
  ...
  (testing "the function return-more-words-like-this"
           (with-redefs [dictionary/find-words-like-this (fn [input]
                                                           ["one" "two"])]
...
(deftest test-get-input
  ...
  (testing "the function get-input"
           ...
           (are [x in-channel? input-text]
                (= x (get-input in-channel? input-text))
                "" false
                "" "test"
                false "test"
                "test" true
...
(deftest test-prepare-reply-text-unknown-command
  ...
  (testing
    "the function prepare-reply-text"
    (reset! dyncfg/configuration {:bot {:prefix "?", :help "help message"}})
    (reset! dyncfg/bot-nick "bot")
    (is (= (prepare-reply-text {:target ""} "" "xyzzy")
           {:prefix nil, :response "Command not understood or term not found"}))
    (is (= (prepare-reply-text {:target ""} "bot" "xyzzy")
           {:prefix nil,
            :response "Command not understood or term not found"}))))

(deftest test-prepare-reply-text-help-message
  "Check the behaviour of function clj-dopey.irc-bot/prepare-reply-text"
  (testing "the function prepare-reply-text"
           (reset! dyncfg/configuration
                   {:bot {:prefix "?", :help "help message"}})
           (reset! dyncfg/bot-nick "bot")
           (is (= (prepare-reply-text {:target ""} "" "help")
                  {:prefix nil, :response "help message"}))
           (is (= (prepare-reply-text {:target ""} "bot" "help")
                  {:prefix nil, :response "help message"}))
           (is (= (prepare-reply-text {:target "#channel"} "bot" "bot: help")
                  {:prefix "bot: ", :response "help message"}))))
...
(deftest test-prepare-reply-text-status
  ...
  (testing
    ...
    (with-redefs [dictionary-status (fn [] "status reply")]
      (is (= (prepare-reply-text {:target ""} "" "status")
             {:prefix nil, :response "status reply"}))
      (is (= (prepare-reply-text {:target ""} "bot" "status")
             {:prefix nil, :response "status reply"}))
      (is (= (prepare-reply-text {:target "#channel"} "bot" "bot: status")
             {:prefix "bot: ", :response "status reply"})))))
...
(deftest test-prepare-reply-text-rainbow
  ...
  (testing
    ...
    (with-redefs [get-rainbow-as-message (fn [] "rainbow reply")]
      (is (= (prepare-reply-text {:target ""} "" "rainbow")
             {:prefix nil, :response "rainbow reply"}))
      (is (= (prepare-reply-text {:target ""} "bot" "rainbow")
             {:prefix nil, :response "rainbow reply"}))
      (is (= (prepare-reply-text {:target "#channel"} "bot" "bot: rainbow")
             {:prefix "bot: ", :response "rainbow reply"})))))
...
(deftest test-prepare-reply-text-exception-handling
  ...
  (testing
    ...
    (is (= (prepare-reply-text {:target ""} nil nil)
           {:prefix "", :response ""}))
